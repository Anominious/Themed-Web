...
A bonding box of a section is decided by its largest and smallest values in x, y, and z direction. The length, width, and height of a bonding box can be obtained by doing subtractions bewteen each pair of x, y, and z values. 
Bonding Box Coordinates Examples:
<img src="img/bbox.png" width="300"/>

#### Aspect Ratio

The [aspect ratio](https://en.wikipedia.org/wiki/Aspect_ratio) the ratio between the length and width of a shape. Since the bonding box of a section is a rectangular prism, this project takes 3 aspect ratio of each section: xy, xz, yz. 
...
```
Next, remove the parent sections, which only serve as folders for other sections, and are not actually drawn. The bonding box values for these sections are all -1.
```matlab
probelm = [-1 -1 -1 -1 -1 -1];
...
```
Also calculate the distance between a volxel and each of its neighbors, based on pixel length, pixel width, and page thickness. 
```matlab
...
**Extracting Bonding box:**

Since the bonding box's side lengths are number of pixels, to calculate the true ratio of each mitochondria, the length needed to be multiplied with the pixel_length, as the width with pixel_width and height with page_thickness. After the ratio is calculated, they are appended to an array as a vector. 
```matlab
